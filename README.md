...
- **ランダム値生成機能**（`@faker-js/faker` によるテストデータ自動生成）
  - 各入力フィールドに「生成」ボタンを追加
  - フィールドのラベルから適切な faker メソッドを自動推測
  - 氏名、メール、電話番号、住所など50種類以上のフィールドタイプに対応
  - 対応フィールド: firstName, lastName, email, phone, address, zipCode, company, username, password, url, uuid など
- 複数戦略による要素探索
  - CSS セレクタ
...
- `fieldGenerators.ts`: フィールド名に基づく faker メソッドの自動選択（50+種類のマッピング）
